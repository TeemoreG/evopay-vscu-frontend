...
const ZReport = ({ data, loading, dateRange }) => {
  ...
  const downloadPDF = () => {
    try {
      ...
      doc.setTextColor(255, 255, 255);
      doc.text('Z Report - Daily Close', pageWidth / 2, 28, { align: 'center' });

      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(200, 200, 200);
      doc.text(`Period: ${dateRange?.start || 'N/A'} to ${dateRange?.end || 'N/A'}`, pageWidth / 2, 36, { align: 'center' });
      ...
      doc.text('Z REPORT SUMMARY', 14, yPos);
      yPos += 6;
      ...
      doc.setFontSize(7);
      doc.setTextColor(200, 200, 200);
      doc.text('This Z Report is KRA eTIMS compliant.', pageWidth / 2, finalY + 8, { align: 'center' });
      doc.text('This report represents the daily close of all sales operations.', pageWidth / 2, finalY + 14, { align: 'center' });
      doc.text('© Evopay Limited. All rights reserved.', pageWidth / 2, finalY + 20, { align: 'center' });

      doc.save('z-report.pdf');
    } catch (error) {
  ...
  const exportCSV = () => {
    try {
      ...
      a.href = url;
      a.download = `z-report_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-[#1a2a4a]">Z Report - Daily Close</h2>
      ...
      <div className="bg-gray-50 p-3 rounded mb-4 text-sm text-gray-600">
        <span className="font-medium">Period:</span> {dateRange?.start || 'N/A'} to {dateRange?.end || 'N/A'}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <p className="text-sm text-gray-500">NS Receipts</p>
          <p className="text-2xl font-bold text-[#1a2a4a]">{data.totalSales || 0}</p>
        </div>
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <p className="text-sm text-gray-500">NC Receipts</p>
